...
// getting exchange rate data from 3rd party API
function calculate() {
    const valueOne = currencyOne.value;
    const valueTwo = currencyTwo.value;
    document.getElementById("curname").innerHTML = currencyTwo.value;
    fetch(`https://api.exchangerate-api.com/v4/latest/${valueOne}`)
        .then(res => res.json())
        .then(({ rates }) => {
            amountTwo.innerText = (rates[valueTwo] * amountOne.value).toFixed(2);
